...
class VectorQuantiser(nn.Module):
    ...
    @torch.no_grad()
    def _ema_update(self, enc, flat):
        self.ema_cluster_size.mul_(self.decay).add_(enc.sum(0), alpha=1-self.decay)
        self.ema_weight_sum.mul_(self.decay).add_(enc.T @ flat, alpha=1-self.decay)
        n = self.ema_cluster_size.sum()
        cluster_size = (self.ema_cluster_size + self.eps) / (n + self.K*self.eps)
        self.codebook.data.copy_(self.ema_weight_sum / cluster_size.unsqueeze(1))
...
class ByteSegmentCompressor(nn.Module):
    ...
    def forward(self, token_ids: torch.Tensor,
        ...
        with torch.no_grad():
            ent   = token_entropy(logits)               # (B,S)
            segid = entropy_segments(ent)               # (B,S)
        ...
        pooled, _ = self.pooler(hidden, attn_mask=mask,
