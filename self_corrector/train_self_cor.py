...
from self_correcting import SelfCorrectingTransformerLayer
...
def generate(model, tokenizer, prompt: str, max_new_tokens: int = 64, temperature: float = 1.0,
...
def evaluate_perplexity(model, token_chunks: List[torch.Tensor], device, vocab_size) -> float:
...
def main():
    ...
    parser.add_argument("--layer_repetition", type=int, default=2)
    ...
    if Run is not None:
        aim_run = Run(experiment="self_corrector",)
    ...
    model =  nn.Sequential(
        nn.Embedding(tokenizer.vocab_size, args.model_dim),
        *[SelfCorrectingTransformerLayer(
        ...
        nn.Linear(args.model_dim, tokenizer.vocab_size)
    ).to(device)
    ...
    while global_step < args.steps:
        ...
        for micro in range(args.grad_accum):
            ...
            micro_loss = (loss_main) / args.grad_accum
            tok_seen += B * S
            micro_loss.backward()
